...
const AIChat = ({ isOpen, onClose }: AIChatProps) => {
  ...
  const callOpenRouter = async (history: { role: "user" | "assistant"; content: string }[]) => {
    ...
    const modelFromEnv = (import.meta.env.VITE_OPENROUTER_MODEL as string) || "openai/gpt-4o-mini";
  ...
  return (
    <div className={cn(
      ...
      <Card className="h-full glass-effect border-l-2 border-primary/20 shadow-floating">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b bg-gradient-ai">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-semibold text-primary-foreground">ИИ-Помощник</h2>
              <p className="text-xs text-primary-foreground/80">Всегда онлайн</p>
            </div>
          </div>
          <Button
            variant="ghost"
